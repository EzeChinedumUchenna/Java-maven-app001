                ...
                script {
                   ...
                   // }
                    sshagent(['Production_Server_SSH-KEY']) {
                        def dockerCmd = "docker run -p 8081:8080 -d nedumacr.azurecr.io/demo-app:jma-$IMAGE_NAME" // Make sure you have docker and az CLI installed on both the Production Server and Jenkins server and that Java-maven port uns on port 8080
                        ...
                        sh "docker run -p 8081:8080 -d nedumacr.azurecr.io/demo-app:jma-$IMAGE_NAME"
                    } 
                }
            }
        }
